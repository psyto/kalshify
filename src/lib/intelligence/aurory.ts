/**
 * Aurory Intelligence Data Fetcher
 */
...
const AURORY_PROGRAM_ID = "AURYxxLd8c4qFd3s2Q1a1n4ZWLJ7XfyqTxq6HgFjE1"; // Aurory main program

export async function fetchAuroryData(): Promise<IntelligenceData> {
    try {
        console.log("Fetching Aurory intelligence data...");
        const [github, twitter, onchain] = await Promise.all([
            getOrganizationMetrics("AuroryProject").catch(() => ({
                totalContributors: 0,
            ...
            getEngagementMetrics("AuroryProject").catch((err) => {
                throw err;
            }),
            getOnChainMetrics(AURORY_PROGRAM_ID).catch(() => ({
                contractAddress: AURORY_PROGRAM_ID,
                chain: "solana",
        ...
        console.error("Error fetching Aurory data:", error);
        throw error;
    }
}

export async function calculateAuroryScore(
    data?: IntelligenceData
): Promise<IntelligenceScore> {
    const intelligenceData = data || (await fetchAuroryData());
...
export async function getAuroryCompanyData(
    data?: IntelligenceData,
    score?: IntelligenceScore
): Promise<Company> {
    const intelligenceData = data || (await fetchAuroryData());
    const intelligenceScore =
        score || (await calculateAuroryScore(intelligenceData));
