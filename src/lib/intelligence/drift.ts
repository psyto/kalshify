/**
 * Drift Protocol Intelligence Data Fetcher
 */
...
const DRIFT_PROGRAM_ID = "dRiftyHA39bWEY4nXUvNnprgNa8gvhcteAdT";

export async function fetchDriftData(): Promise<IntelligenceData> {
    try {
        console.log("Fetching Drift intelligence data...");
        const [github, twitter, onchain] = await Promise.all([
            getOrganizationMetrics("drift-labs").catch(() => ({
                totalContributors: 0,
...
export async function calculateDriftScore(
    ...
    const intelligenceData = data || (await fetchDriftData());
...
export async function getDriftCompanyData(
    data?: IntelligenceData,
    score?: IntelligenceScore
): Promise<Company> {
    const intelligenceData = data || (await fetchDriftData());
    const intelligenceScore =
        score || (await calculateDriftScore(intelligenceData));
