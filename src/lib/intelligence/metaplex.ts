/**
 * Metaplex Intelligence Data Fetcher
 */
...
export async function fetchMetaplexData(): Promise<IntelligenceData> {
    try {
        console.log("Fetching Metaplex intelligence data...");
        const [github, twitter, onchain] = await Promise.all([
            getOrganizationMetrics("metaplex-foundation").catch(() => ({
                totalContributors: 0,
            ...
            getEngagementMetrics("metaplex").catch((err) => {
                throw err;
            }),
            getOnChainMetrics(METAPLEX_PROGRAM_ID).catch(() => ({
                contractAddress: METAPLEX_PROGRAM_ID,
                chain: "solana",
        ...
        console.error("Error fetching Metaplex data:", error);
        throw error;
    }
}

export async function calculateMetaplexScore(
    data?: IntelligenceData
): Promise<IntelligenceScore> {
    const intelligenceData = data || (await fetchMetaplexData());
...
export async function getMetaplexCompanyData(
    data?: IntelligenceData,
    score?: IntelligenceScore
): Promise<Company> {
    const intelligenceData = data || (await fetchMetaplexData());
    const intelligenceScore =
        score || (await calculateMetaplexScore(intelligenceData));
