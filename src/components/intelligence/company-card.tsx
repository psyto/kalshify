...
export function CompanyCard({ company }: CompanyCardProps) {
  ...
  return (
    <Link href={`/intelligence/${company.slug}`}>
      <div className="group bg-card rounded-lg border border-border p-6 transition-all hover:border-purple-300 hover:shadow-lg">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className={cn('text-xs px-2 py-0.5 rounded-full', categoryColors[company.category])}>
                {company.category.toUpperCase()}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <TrendIcon className={cn('h-4 w-4', trendColors[company.trend])} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <p className={cn('text-2xl font-bold', scoreColor)}>{company.overallScore}</p>
          </div>
          <div>
        ...
        <div className="space-y-2 text-xs text-muted-foreground border-t border-border pt-4">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-foreground">+{company.growth.userGrowthRate}%</span>
          </div>
          {company.isListed && (
            <div className="flex justify-between">
              <span className="text-purple-600 font-medium">Listed in Marketplace</span>
