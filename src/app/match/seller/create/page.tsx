...
import { IntelligenceLinkStep } from '@/components/forms/listing/intelligence-link-step';
import type { CreateListingInput, ListingTypeEnum } from '@/lib/schemas/listing';
import { z } from 'zod';
...
export default function CreateListingPage() {
  ...
  if (!authLoading && !isConnected) {
    router.push('/match?error=wallet_required');
    return null;
  }
  ...
  const handleSubmit = async () => {
    try {
      ...
      router.push(`/match/seller/listings/${listing.id}`);
    } catch (error) {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 3:
        return (
          <MetricsStep
            listingType={formData.type as z.infer<typeof ListingTypeEnum>}
            defaultValues={formData}
            onNext={(data) => {
              updateFormData(data);
              handleNext();
      ...
      <div className="container max-w-4xl mx-auto py-8 px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            List your project for acquisition, investment, or partnership opportunities
          </p>
        </div>
